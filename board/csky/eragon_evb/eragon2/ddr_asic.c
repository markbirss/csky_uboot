...
//version       :1.19
//note          :
//1.map lcdc axi port to vpr to optimize QoS to avoid underflow for DDR3_1066_G
//
//
//updater       :Wu Youfei
//date          :2017/9/25
//version       :1.20
//note          :
//1.map audio axi port to vpr to optimize QoS to avoid underflow for DDR3_1066_G
//2.delete now
//
//updater       :Wu Youfei
//date          :2017/9/29
//version       :1.21
//note          :
//1.modify tRFCmin=260ns for DDR3_1333_H/DDR3_1600_K
//
//updater       :Wu Youfei
//date          :2017/12/22
//version       :1.22
//note          :
//1.modify ADDRMAP of DLL_OFF_200 and DDR3_1066_G to improve access efficiency
//
//updater       :Wu Youfei
//date          :2018/2/25
//version       :1.23
//note          :
//1.modify PUB_PTR3/PUB_PTR4 for DDR3_1333_H and DDR3_1600_K to real time(200us/500us) for board test
//////////////////////////////////////////////////////////////////////////////
...
int init_ddr(){
*(volatile int *)_FLAG_DDR_INIT=0x0;//do not distinguish for
#ifdef FPGA
//if(PUB_REG(PUB_PGSR0)==0x9000005f){return 0;}//if DDR is initialized,return
     ...
     //  DDR_REG(DIMMCTL)=0x00000000;
      ...
      DDR_REG(ADDRMAP2)=0x00000000;//c5 c4 c3 c2
      DDR_REG(ADDRMAP3)=0x00000000;//c9 c8 c7 c6
      DDR_REG(ADDRMAP4)=0x00000f0f;//c11 c10(not used,so config f)
      DDR_REG(ADDRMAP5)=0x07070707;//r11 r10-r2 r1 r0
      DDR_REG(ADDRMAP6)=0x07070707;//r15 r14 r13 r12
...
if((*((volatile int *)_FLAG_DDR_INIT))==0x0){
  ...
  CHIP_CTRL_REG(0x40)=CHIP_CTRL_REG(0x40)&0xdffffff;


}

  #if defined(DDR3_1066_G)
  DMEM_REG(0xffc)=0x5a5a5a5a;
...
//    DDR_REG(HWLPCTL)=0x006e0002;
    ...
    DDR_REG(RFSHTMG)=0x00400046;//tFRCmin=260ns
//    DDR_REG(CRCPARCTL0)=0x00000000;
...
//    DDR_REG(POISONCFG)=0x00010011;
 ...
 //NA
    //****************config QoS of axi port*************//
    //***map audio port to vpr******//
    //DDR_REG(PCFGQOS0_3)=0x00100000;//map region1 to vpr
    //DDR_REG(PCFGQOS1_3)=0x00000004;//bule queue timeout value=4
...
//    DDR_REG(PCTRL_5)=0x00000001;
   ...
   //**************de-reset all other reset *****************//
    ...
    PUB_REG(PUB_DTPR1)=0x1a846360;//tRFCmin=260ns
   ...
   // }
    ...
    if(*((volatile int *)_FLAG_DDR_INIT)==0x0){ //default do sdram initialization
        //************data training configuration**********************//
         //PUB_REG(PUB_DTCR)=PUB_REG(PUB_DTCR)&0xfffffff0|0x41;//DTRPTN=1 DTMPR=1
         PUB_REG(PUB_DTCR)=PUB_REG(PUB_DTCR)&0xfffffff0|0x41;//DTRPTN=1
         DDR_REG(RFSHCTL3)=0x00000001;//disable auto-refresh

         //PUB_REG(PUB_PIR)= 0x00000401;//run Training
         //while((PUB_REG(PUB_PGSR0))!=0x9000005f);
  ...
  //}
    ...
    if(*((volatile int *)_FLAG_DDR_INIT)==0x1){
         PUB_REG(PUB_DTCR)=PUB_REG(PUB_DTCR)&0xfffffff0|0x41;//DTRPTN=1
         DDR_REG(RFSHCTL3)=0x00000001;//disable auto-refresh

         //PUB_REG(PUB_PIR)= 0x00000401;//run read dqs training after exit from self-refresh
         PUB_REG(PUB_PIR)= 0x00000e72;//run read dqs training after exit from self-refresh
         while((PUB_REG(PUB_PGSR0))!=0x9000005f);
...
//    DDR_REG(HWLPCTL)=0x006e0002;
    ...
    DDR_REG(RFSHCTL0)=0x00e06010;//modify refresh_to_x32=0x6
    DDR_REG(RFSHCTL3)=0x00000000;
    DDR_REG(RFSHTMG)=0x00510057;//tRFCmin=260ns 4096Mbx16 device
//    DDR_REG(CRCPARCTL0)=0x00000000;
    if((*((volatile int *)_FLAG_DDR_INIT))==0x0){
        DDR_REG(INIT0)=0x40080003;
    }
...
//    DDR_REG(PCTRL_5)=0x00000001;
   ...
   //**************de-reset all other reset *****************//
    ...
    PUB_REG(PUB_DTPR1)=0x1a8573c0;//[19:11]tRFCmin=260ns 4096Mbx16 device
    ...
    PUB_REG(PUB_PTR3)=0x0b8516f4;//[19:0]*tCK=500us
    //PUB_REG(PUB_PTR4)=0x08000c00;
    PUB_REG(PUB_PTR4)=0x0802092f;//[17:0]*tCK=200us
  ...
  DDR_REG(RFSHTMG)=0x00610068;//[9:0] tRFCmin=260ns 4096Mbx16 device
  if((*((volatile int *)_FLAG_DDR_INIT))==0x0){
      DDR_REG(INIT0)=0x40080003;
  }
  else{
...
//  DDR_REG(PCTRL_5)=0x00000001;
 ...
 //PUB_REG(PUB_PTR3)=0x0a100a46;//for reduce simulation time,config smaller value than 500us,which will lead to timing violation in ddr3 modle(ddr3.v)
    PUB_REG(PUB_PTR3)=0x0a161a80;//tDINIT0=500us=PUB_PTR3[19:0]*tCK

 //PUB_REG(PUB_PTR4)=0x08000c00;//for reduce simulation time,config smaller value than 200us,which will lead to timing violation in ddr3 modle(ddr3.v)
    PUB_REG(PUB_PTR4)=0x08027100;//tDINIT2=200us=PUB_PTR4[17:0]*tCK(reference PUB databook)
  ...
  PUB_REG(PUB_DTPR1)=0x1a8d0c00;//[19:11] tRFCmin=260ns 4096Mbx16 device
  ...
  *((volatile int *)_RETENTION_CTRL_REG)=0x0;//remove latch to SDRAM before exit self-refresh
  ...
  while((DDR_REG(STAT))!=0x00000001);
  ...
  #elif defined(DDR3_800_E)
      DMEM_REG(0xffc)=0x88888888;
      ...
      //******************start configuration******************//
        ...
        DDR_REG(RFSHCTL3)=0x00000000;
        //DDR_REG(RFSHTMG)=0x00300020;
        DDR_REG(RFSHTMG)=0x00300044;//tRFCmin=260ns 4096Mbx16 device
        if((*((volatile int *)_FLAG_DDR_INIT))==0x0){
            DDR_REG(INIT0)=0x40080003;
        }
        else{
      ...
      //  DDR_REG(DIMMCTL)=0x00000000;
        ...
        //******config hif addr map to sdram addr(ba,row,col)************//
         //DDR_REG(ADDRMAP1)=0x00161616;//b2 b1 b0
         DDR_REG(ADDRMAP1)=0x00171717;//b2 b1 b0 //4096Mbx16
         DDR_REG(ADDRMAP2)=0x00000000;//b5 b4 b3 b2
         DDR_REG(ADDRMAP3)=0x00000000;//b9 b8 b7 b6
         DDR_REG(ADDRMAP4)=0x00000f0f;//b11 b10(not used,so config f)
         DDR_REG(ADDRMAP5)=0x04040404;//r11 r10-r2 r1 r0
         //DDR_REG(ADDRMAP6)=0x0f0f0404;//r15 r14(not used,so config f) r13 r12
         DDR_REG(ADDRMAP6)=0x0f040404;//r15 (not used,so config f)r14 r13 r12
      ...
      //  DDR_REG(SCHED)=0x303a0501;
        ...
        //****************config QoS of axi port*************//
        DDR_REG(PCFGQOS0_2)=0x00100000;//map region1 to vpr
        DDR_REG(PCFGQOS1_2)=0x00000008;//blue queue timout value=8
...
//        DDR_REG(PCTRL_5)=0x00000001;
      //**************de-reset all other reset *****************//
       ...
       CHIP_CTRL_REG(0x3c)=CHIP_CTRL_REG(0x3c)|0xffff0000;
        ...
        PUB_REG(PUB_DTPR0)=0x550f6644;
        //PUB_REG(PUB_DTPR1)=0x1a820280;
        PUB_REG(PUB_DTPR1)=0x1a844280;//tRFCmin=260ns 4096Mbx16 device
        PUB_REG(PUB_DTPR2)=0x10018c03;
      ...
      //  PUB_REG(PUB_PGCR2)=0x00f80c30;//Note :not support fixed latency mode in emulation(for FPGA) PHY
        PUB_REG(PUB_PGCR2)=0x00f00c30;
       // PUB_REG(PUB_PTR3)=0x1e3009e3;//
        PUB_REG(PUB_PTR3)=0x1e330d40;//500us
        //PUB_REG(PUB_PTR4)=0x08000c00;
        PUB_REG(PUB_PTR4)=0x08013880;//200us
  ...
  if(*((volatile int *)_FLAG_DDR_INIT)==0x0){ //default do sdram initialization
  ...
  #else
    if(*((volatile int *)_FLAG_DDR_INIT)!=0x0){ //if power up from low power mode,the status is not checkend before,so check now
        //*************wait PHY initialization Done*****************//
        while((PUB_REG(PUB_PGSR0))!=0x9000001f);
    }
  #endif
    ...
    #ifdef GATESIM
        DMEM_REG(DDR_INIT_MAIL_BOX)=0x2;//training finish inform
    #endif
