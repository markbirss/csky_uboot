...
#define dev_err(dev, fmt, args...) { \
	printf("\t%s - error: ", (dev) == NULL ? \
	       "mailbox-x" : ((struct mbox_shell_device *)(dev))->name); \
	printf(fmt, ##args); \
}
...
#define SHELL_MBOX_DEV_ID0	0
#define SHELL_MBOX_DEV_ID1	1
#define SHELL_MBOX_DEV_COUNT	2
...
enum mbox_shell_mssg_type {
	SHELL_MBOX_MSSG_DATA = 'd',	/* Data to receiver */
	SHELL_MBOX_MSSG_ACK  = 'a',	/* ACK to sender */
};


/**
 * struct mbox_message - Description of a message that send to mailbox
 * @mssg_type:	The message type that transfer, refer to mbox_shell_mssg_type
 * @length:	Then data length, must <= SHELL_MBOX_MAX_DATA_LENGTH
 * @reserved0:	Undefined
 * @reserved1:	Undefined
 * @data:	The transfer data. Ignore if mssg_type is SHELL_MBOX_MSSG_ACK
 */
struct mbox_shell_message {
	u8 mssg_type;
	u8 length;
	u8 reserved0;
	u8 reserved1;
	u8 data[SHELL_MBOX_MAX_DATA_LENGTH];
};

/**
 * struct mbox_shell_device - SHELL mailbox device internal informations
 * @name:	Name of device
 * @dev_id:	Device ID
 * @base:	Base address of this device
 */
struct mbox_shell_device {
	const char	*name;
	uint		dev_id;
	void 		*base;
	bool		is_open;
...
static struct mbox_shell_device mboxes[SHELL_MBOX_DEV_COUNT];
static const char* device_names[SHELL_MBOX_DEV_COUNT] = {
	"mailbox-0", "mailbox-1"
};

static int mbox_get_closed_dev(struct mbox_shell_device **mbox, uint dev_id)
{
	*mbox = NULL;
	if (dev_id != SHELL_MBOX_DEV_ID0 && dev_id != SHELL_MBOX_DEV_ID1) {
...
static int mbox_get_opened_dev(struct mbox_shell_device **mbox, uint dev_id)
{
	*mbox = NULL;
	if (dev_id != SHELL_MBOX_DEV_ID0 && dev_id != SHELL_MBOX_DEV_ID1) {
		dev_err(*mbox, "Get failed, invalid dev_id(%u)\n", dev_id);
...
{
	int i;
	for (i = 0; i < SHELL_MBOX_DEV_COUNT; ++i) {
...
{
	struct mbox_shell_device *mbox = NULL;
	int i, ret;
	...
	/* Clear RX buffer */
	for(i = 0; i < SHELL_MBOX_MAX_MESSAGE_LENGTH / sizeof(uint); ++i) {
...
{
	struct mbox_shell_device *mbox = NULL;
	struct mbox_shell_message mssg;
	...
	tx_len = min(length, (uint)SHELL_MBOX_MAX_DATA_LENGTH);
	mssg.mssg_type = SHELL_MBOX_MSSG_DATA;
	mssg.length = tx_len;
	mssg.reserved0 = 0;
	mssg.reserved1 = 0;
	mssg_total_len = SHELL_MBOX_MSSG_HEAD_LENGTH + mssg.length;
...
int mailbox_shell_recv(uint dev_id, char *data, uint length)
{
	 ...
	 */
	if (length < SHELL_MBOX_MAX_MESSAGE_LENGTH) {
	...
	memcpy(&mssg, MBOX_RX_MSSG_ADDR(mbox), SHELL_MBOX_MAX_MESSAGE_LENGTH);
	RX_CLEAR_INTERRUPT(mbox);

	if (mssg.mssg_type == SHELL_MBOX_MSSG_DATA &&
	    mssg.length <= SHELL_MBOX_MAX_DATA_LENGTH) {
		...
		struct mbox_shell_message *mssg_tx =
			(struct mbox_shell_message*)MBOX_TX_MSSG_ADDR(mbox);
		mssg_tx->mssg_type = SHELL_MBOX_MSSG_ACK;
		TX_GENERATE_INTERRUPT(mbox);
...
#endif
		return mssg.length;
	} else if (mssg.mssg_type == SHELL_MBOX_MSSG_ACK) {
